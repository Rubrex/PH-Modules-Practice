const assignmentGrid = () => {
  ...
  assignmentAmount =
    assignmentAmount > 0 && assignmentAmount <= 99 ? assignmentAmount : 12;

  assignmentContainer.innerHTML = "";

  for (let i = 0; i < assignmentAmount; i++) {
    const createdDiv = document.createElement("div");
    createdDiv.classList.add(
      "bg-green-400",
      "rounded",
      "p-5",
      "flex",
    ...
    createdDiv.innerHTML = `
            <h2 class="text-2xl font-semibold text-center">Assignment ${
              i + 1
            }</h2>
...
const calculate = () => {
  ...
  for (const assignment of assignmentContainer.children) {
    ...
    if (obtained !== 0 && (obtained / submission) * 100 < 50) {
      lessHalf = true;
    }
  }
  ...
  const answer =
    gapMarks > 0 && restAssignments !== 0
      ? (gapMarks / restAssignments).toFixed(2)
      : gapMarks <= 0
      ? "Fulfilled"
      : "Not Fulfilled";

  const lessHalfMsg =
    '<span class="text-red-400 font-semibold"> Oops! You got less than 50% marks in minimum one assignment.</span>';
  const overLimitMsg = `<span class="text-red-400 font-semibold">Oops! It\'s not possible to get ${answer} out of 60'.</span>`;
  const halfOkMsg =
    'you should not get less than <span class="text-yellow-200 font-semibold">50%</span> marks in any rest of assignments.';
  const limitOkMsg = `You need to get at least <span class="text-lime-400 font-semibold">${Math.ceil(
    answer
  )}</span> marks in each rest of assignments.`;
  ...
  result.innerHTML = `
        ...
        <h3 class="text-2xl text-center text-${
          answer > 60 || answer === "Not Fulfilled" ? "red" : "lime"
        }-400 font-semibold my-3">${answer}</h3>
        <p class="text-center">${
          answer > 60
            ? overLimitMsg
            : answer === "Fulfilled" && !lessHalf
            ? fulfilledMsg
            : answer === "Fulfilled" && lessHalf
            ? "But"
            : answer === "Not Fulfilled"
            ? notfulfilledMsg
            : limitOkMsg
        } <br> ${
    lessHalf
